    ...
    items: [
      {
        ...
        a: 'Clique sur "Vendre", entre l\'ISBN au dos du livre pour auto-remplir titre et auteur, ajoute une photo, fixe ton prix et publie. C\'est en ligne immédiatement.'
      },
      {
...
export default function ManuelsFaqView({ setView }) {
  return (
    <div style={{ maxWidth: 720 }}>
      ...
      <div style={{ textAlign: 'center', marginBottom: 28 }}>
        ...
        <h1 style={{ fontSize: 23, fontWeight: 900, color: '#1a2e4a', margin: '0 0 8px' }}>FAQ Manuels</h1>
        <p style={{ fontSize: 14, color: '#64748b', margin: 0 }}>Toutes les réponses à tes questions sur l'achat et la vente de manuels.</p>
      </div>
      ...
      <div style={{ background: 'linear-gradient(135deg,#1a2e4a,#2d4a6b)', borderRadius: 14, padding: '24px', textAlign: 'center' }}>
        ...
        <div style={{ display: 'flex', gap: 10, justifyContent: 'center', flexWrap: 'wrap' }}>
          <button onClick={() => setView('acheter')} style={{ padding: '10px 20px', background: 'rgba(255,255,255,0.15)', color: 'white', border: '1px solid rgba(255,255,255,0.3)', borderRadius: 8, fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>
            Voir les manuels
          </button>
          <button onClick={() => setView('vendre')} style={{ padding: '10px 20px', background: '#00c9a7', color: 'white', border: 'none', borderRadius: 8, fontSize: 13, fontWeight: 700, cursor: 'pointer' }}>
            Vendre un manuel →
          </button>
        </div>
      </div>

...
export default function TuteursFaqView({ setView }) {
  return (
    <div style={{ maxWidth: 720 }}>
      ...
      <div style={{ textAlign: 'center', marginBottom: 28 }}>
        ...
        <h1 style={{ fontSize: 23, fontWeight: 900, color: '#1a2e4a', margin: '0 0 8px' }}>FAQ Tuteurs</h1>
        <p style={{ fontSize: 14, color: '#64748b', margin: 0 }}>Toutes les réponses à tes questions sur le tutorat entre étudiants.</p>
      </div>
      ...
      <div style={{ background: 'linear-gradient(135deg,#1a2e4a,#2d4a6b)', borderRadius: 14, padding: '24px', textAlign: 'center' }}>
        ...
        <div style={{ display: 'flex', gap: 10, justifyContent: 'center', flexWrap: 'wrap' }}>
          <button onClick={() => setView('tuteurs')} style={{ padding: '10px 20px', background: 'rgba(255,255,255,0.15)', color: 'white', border: '1px solid rgba(255,255,255,0.3)', borderRadius: 8, fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>
            Voir les tuteurs
          </button>
          <button onClick={() => setView('devenir-tuteur')} style={{ padding: '10px 20px', background: '#00c9a7', color: 'white', border: 'none', borderRadius: 8, fontSize: 13, fontWeight: 700, cursor: 'pointer' }}>
            Devenir tuteur →
          </button>
        </div>
      </div>

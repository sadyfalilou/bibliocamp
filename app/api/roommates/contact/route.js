...
// POST { roommate_listing_id, owner_id } — trouve ou crée une conversation, retourne { conversation_id }
export async function POST(request) {
  const { roommate_listing_id, owner_id } = await request.json()
  if (!roommate_listing_id || !owner_id) {
    return NextResponse.json({ error: 'roommate_listing_id et owner_id requis' }, { status: 400 })
  }
